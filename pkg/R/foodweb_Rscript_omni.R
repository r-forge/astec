...
Ranimal_growth2_logbodymass_without_waste<-function(animal_density,animal_logbodymass,animal_bodymass_average,food_assimilated,n_mass,metabolic_parameters,deltaT){
 res=animal_density
 respiration=animal_density*Ranimal_respiration(animal_bodymass_average,metabolic_parameters,deltaT)
 for (i in 1:n_mass){
  if (animal_density[i]>0.0){
...
Ranimal_growth2_logbodymass_matrix_without_waste<-function(animal_density_list,animal_logbodymass_list,animal_bodymass_average_list,food_assimilated_list,n_mass_list,metabolic_parameters_matrix,n_trophic_group,deltaT){
 res=vector("list",n_trophic_group)
 for (i in 1:n_trophic_group){
  res[[i]]=Ranimal_growth2_logbodymass_without_waste(animal_density_list[[i]],animal_logbodymass_list[[i]],animal_bodymass_average_list[[i]],food_assimilated_list[[i]],n_mass_list[i],metabolic_parameters_matrix[i,],deltaT)
 }
 res
}
...
Rcompute_attack_rate_omni<-function(animal_bodymass_average_list,predation_parameters,herbivory_parameters,detritivory_parameters,n_trophic_group,n_mass_list){
 ...
 for (i in 1:n_trophic_group){
  for (j in 1:n_mass_list[i]){
   ...
   for (k in 1:n_trophic_group){
    for (l in 1:n_mass_list[k]){
     attack_rate_matrix[p1,p2]=predation_parameters[i,1]*animal_bodymass_average_list[[i]][j]*exp(-((log(animal_bodymass_average_list[[k]][l]/animal_bodymass_average_list[[i]][j])-log(predation_parameters[i,2]))/predation_parameters[i,3])^2)
     p2=p2+1
    }
   }
...
Rcompute_handling_time_omni<-function(animal_bodymass_average_list,predation_parameters,herbivory_parameters,detritivory_parameters,n_trophic_group,n_mass_list){
    # from Harfoot et al. 2014 - eq.40
...
Rcomputation_consumption_matrix<-function(animal_density_list,biomass_plant,biomass_detritus,attack_rate_matrix,handling_time_matrix,n_mass_list,pos_density_matrix,n_trophic_group){
 # computation of F_i,j(t) (eq. 47)
 sml=sum(n_mass_list)
 consumption_matrix=matrix(0,sml,(sml+2))
 sum_consumption_array=array(0,(sml+2))
 p1=1
 for (k in 1:n_trophic_group){
  for (l in 1:n_mass_list[k]){
   # computation of the denominator and storage in temp : 1 + sum_m a_im * d_m * T_im
   ...
   temp=temp+(biomass_plant*attack_rate_matrix[p1,(sml+1)]*handling_time_matrix[p1,(sml+1)])
   temp=temp+(biomass_detritus*attack_rate_matrix[p1,(sml+2)]*handling_time_matrix[p1,(sml+2)])
   # computation of F_i,j(t) and sum_predation_matrix [j] = sum_k F_kj
   for (m in 1:(sml+2)){
     consumption_matrix[p1,m]=animal_density_list[[k]][l]*attack_rate_matrix[p1,m]/temp
     sum_consumption_array[m]=(sum_consumption_array[m]+consumption_matrix[p1,m])
   }
   p1=p1+1
  }
 }
 list(consumption_matrix,sum_consumption_array)
}
...
Ranimal_consumption_with_waste<-function(waste_list,animal_density_list,animal_bodymass_average_list,consumption_matrix,sum_consumption_array,deltaT,n_trophic_species,n_mass_list,animal_growth_parameters_matrix,biomass_plant,biomass_detritus){
 sml=sum(n_mass_list)
 newwaste_list=waste_list
 animal_consumed_density_list=vector("list",n_trophic_species)
 animal_newdensity_list=animal_density_list
 food_assimilated_matrix=matrix(0,sml,(sml+2))
 for (k in 1:n_trophic_species){
  animal_consumed_density_list[[k]]=array(0,n_mass_list[k])
 }
 p1=0
 for (k in 1:n_trophic_species){
  for (l in 1:n_mass_list[k]){
   p1=p1+1
   animal_consumed_density_list[[k]][l]=animal_density_list[[k]][l]*(1-exp(-deltaT*sum_consumption_array[p1]))
   animal_newdensity_list[[k]][l]=animal_density_list[[k]][l]-animal_consumed_density_list[[k]][l]
   temp=animal_consumed_density_list[[k]][l]*animal_bodymass_average_list[[k]][l]
   if (sum_consumption_array[p1]>0.0){
    temp=(temp/sum_consumption_array[p1]) # biomass amount since multiplication by average bodymass of prey
   }
   # computation of food eaten by each consumer (eq. 46)
   mc=1
   for (i in 1:n_trophic_species){
    for (j in 1:n_mass_list[i]){
     food_assimilated_matrix[mc,p1]=temp*consumption_matrix[mc,p1] # NB: computation completed two lines below
     newwaste_list[[i]][j]=newwaste_list[[i]][j]+(food_assimilated_matrix[mc,p1]*(1-animal_growth_parameters_matrix[i,1])) # NB : wastes need to be set to 0 at each time step.
     food_assimilated_matrix[mc,p1]=(food_assimilated_matrix[mc,p1]*animal_growth_parameters_matrix[i,1])
     mc=mc+1
    }
   }
  }
 ...
 p1=sml+1
 biomass_plant_consumed=biomass_plant*(1-exp(-deltaT*sum_consumption_array[p1]))
 temp=biomass_plant_consumed
 if (sum_consumption_array[p1]>0.0){
  temp=(temp/sum_consumption_array[p1])
 }
 mc=1
 for (i in 1:n_trophic_species){
  for (j in 1:n_mass_list[i]){
   food_assimilated_matrix[mc,p1]=temp*consumption_matrix[mc,p1] # NB: computation completed two lines below
   newwaste_list[[i]][j]=newwaste_list[[i]][j]+(food_assimilated_matrix[mc,p1]*(1-animal_growth_parameters_matrix[i,2])) # new growth parameter for herbivory/detritivory assimilation
   food_assimilated_matrix[mc,p1]=(food_assimilated_matrix[mc,p1]*animal_growth_parameters_matrix[i,2])
   mc=mc+1
  }
 }

 p1=sml+2
 biomass_detritus_consumed=biomass_detritus*(1-exp(-deltaT*sum_consumption_array[p1]))
 temp=biomass_detritus_consumed
 if (sum_consumption_array[p1]>0.0){
  temp=(temp/sum_consumption_array[p1])
 }
 mc=1
 ...
 new_biomass_plant=biomass_plant-biomass_plant_consumed
 new_biomass_detritus=biomass_detritus-biomass_detritus_consumed

 list(animal_consumed_density_list,animal_newdensity_list,food_assimilated_matrix,newwaste_list,biomass_plant_consumed,biomass_detritus_consumed,new_biomass_plant,new_biomass_detritus)
}
...
Rcompile_consumer_food_eaten<-function(food_assimilated_array,food_assimilated_list,n_trophic_group,n_mass_list){
 sml=sum(n_mass_list)
 newfood_assimilated_list=food_assimilated_list
 l=1
 for (i in 1:n_trophic_group){
...
Rfoodweb_omni<-function(foodweb_inputs){
 ...
 animal_growth_parameters=matrix(0,n_trophic_group,2)
 ...
 for (i in 1:n_trophic_group){
  ...
  animal_growth_parameters[i,2]=trophic_group_parameters[6,i]
  for (j in 1:3){
   metabolic_parameters[i,j]=trophic_group_parameters[(6+j),i]
  }
  mortality_parameters[i,1]=trophic_group_parameters[10,i]
  mortality_parameters[i,2]=trophic_group_parameters[11,i]
  for (j in 1:2){
...
#########################
 ...
 for (istep in 1:n_step_dynamics){
  ...
  #food_assimilated_matrix3d=Rnullify2(n_detritivore,n_mass_list[pos_detritivore],n_detritus) # not used anymore
  ...
  ## computation of consumption matrix F_ij
  temp=Rcomputation_consumption_matrix(animal_density_list,plant_biomass,detritus,attack_rate_matrix,handling_time_matrix,n_mass_list,pos_density_matrix,n_trophic_group)
  consumption_matrix=temp[[1]]
  sum_consumption_array=temp[[2]]

  ## actual consumption (herbivory,carnivory,detritivory and omnivory) and food eaten by each size class and waste computation
  temp=Ranimal_consumption_with_waste(waste_list,animal_density_list,animal_bodymass_average_list,consumption_matrix,sum_consumption_array,deltaT,n_trophic_group,n_mass_list,animal_growth_parameters,plant_biomass,detritus)
  animal_density_list=temp[[2]]
  food_assimilated_matrix=temp[[3]]
  waste_list=temp[[4]]
  plant_consumed=temp[[5]]
  plant_biomass=temp[[7]]
  detritus=temp[[8]]
  food_assimilated_list=Rcompile_consumer_food_eaten(food_assimilated_matrix,food_assimilated_list,n_trophic_group,n_mass_list)
  ...
  if (istep==1){
   ...
   output[[4]]=detritus
   output[[5]]=animal_density_list
   output[[6]]=consumption_matrix
 ...
 ## OUTPUT
 list("animal_density_timeseries"=ad_output,"plant_timeseries"=plant_output,"detritus_timeseries"=det_output,"attack_rate_matrix"=attack_rate_matrix,"handling_time_matrix"=handling_time_matrix,"output"=output,"animal_bodymass_average"=animal_bodymass_average_list,"metabolic_parameters"=metabolic_parameters)
}
